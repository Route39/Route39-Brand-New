...
import {
  DRIVER_TRANSACTIONS_QUERY,
  DRIVER_WALLETS_QUERY,
} from "@/lib/graphql/documents/driver-detail";
import {
  buildOffsetPaging,
  usePageState,
...
export default function DriverFinancialTab() {
  const { driver } = useOutletContext<DriverContext>();
  const { page, pageSize } = usePageState();

  const { data: walletsData, refetch: refetchWallets } = useQuery(DRIVER_WALLETS_QUERY, {
    variables: { driverId: driver.id },
  });
  const { data, loading, error } = useQuery(DRIVER_TRANSACTIONS_QUERY, {
    variables: {
      driverId: driver.id,
  ...
  const wallets = walletsData?.driverWallets.nodes ?? [];
  const transactions = (data?.driverTransactions.nodes ?? []) as TransactionRow[];
  ...
  const columns: DataTableColumn<TransactionRow>[] = [
    ...
    },
    {
      key: "deductType",
      header: "Type",
      cell: (r) => r.deductType ?? r.rechargeType ?? "—",
    },
    {
      key: "status",
      header: "Status",
      cell: (r) => <Badge variant="muted">{r.status}</Badge>,
    },
    {
      key: "amount",
      header: "Amount",
      ...
      <Card>
        ...
        <CardContent className="px-0 pb-0">
          <DataTable
            columns={columns}
            rows={transactions}
            totalCount={data?.driverTransactions.totalCount}
            loading={loading}
