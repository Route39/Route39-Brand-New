...
export class OperatorModule implements OnModuleInit {
  ...
  async onModuleInit() {
    await this.reconcileAdminViewPermissions();
  ...
  private async reconcileAdminViewPermissions(): Promise<void> {
    try {
      const adminRoles = await this.operatorRoleRepository
        .createQueryBuilder('role')
        .where('LOWER(role.title) = :title', { title: 'admin' })
        .getMany();
      if (adminRoles.length === 0) return;

      const viewPerms = Object.values(OperatorPermission).filter((p) =>
        p.endsWith('_View'),
      );

      for (const role of adminRoles) {
        const current = new Set(role.permissions ?? []);
        const missing = viewPerms.filter((p) => !current.has(p));
        if (missing.length === 0) continue;
        role.permissions = [
          ...current,
          ...missing,
        ] as OperatorPermission[];
        await this.operatorRoleRepository.save(role);
      }
    } catch (error) {
      console.error('Failed to reconcile admin view permissions:', error);
    }
  }
}
