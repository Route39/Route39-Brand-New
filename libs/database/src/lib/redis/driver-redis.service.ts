...
export class DriverRedisService {
  ...
  async setLocation(
    ...
    if (driverMetadata) {
      ...
      if (distance < 0.0001) {
        ...
        driverMetadata.location = point;
        driverMetadata.locationTime = new Date();
        pipeline.json.set(key, '$.locationTime', driverMetadata.locationTime.getTime());
  ...
  async makeDriverOnline(input: {
    ...
    const minimumBalance = Number(process.env.DRIVER_MINIMUM_ALLOWED_BALANCE ?? 0);
